(function(b){
	...
	a.prototype._createSilderImg=function(){
		...
		b(c).each(function(f,g){
			...
			b.each(d,function(l,j){
				...
				eleA=null,img=b('<img src="'+j.img+'" alt="'+k+'">');
				if(l==0){
	...
	a.animate=function(g,e,c,f){
		var d=this;clearInterval(g.timer);g.timer=setInterval(function(){
			var i=true;
			for(var h in e){
				var k=parseInt(b(g).css(h)),
				j=(parseInt(e[h])-k)/10;j=j>0?Math.ceil(j):Math.floor(j);
				if(k!=e[h]){
					i=false
				}
				if(h=="zIndex"){
