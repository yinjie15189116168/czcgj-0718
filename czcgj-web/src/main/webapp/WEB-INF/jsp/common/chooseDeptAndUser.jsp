        ...
        $(function () {
            $.ajax({
                ...
                success: function (msg) {
                    ...
                    var userIds = parent.getUserids();
                    var ids = userIds.split(",");
                    for (var idx = ids.length - 1; idx > -1; idx--) {
                        var nodes = zTree.getNodesByParam("id", ids[idx]);
                        if (nodes.length != 0) {
                            for (var i = 0; i < nodes.length; i++) {
                                if (nodes[i].flag == 1) {
                                    var chkNode = nodes[i];
                                    zTree.checkNode(chkNode, true, true);
                                }
                            }
                        }
                    }
...
<div style="width:100%;">
    ...
    <div style="padding-top: 8px;">
        <span style="padding-left: 15px;">姓名：</span> <input id="search"
                                                            type="text" placeholder="全局搜索"/> <input id="search_botton"
                                                                                                    class="ManagerButton"
                                                                                                    type="button"
                                                                                                    value="搜索"
...
<div id="sure"
    ...
    <input type="button" value="确定" class="ManagerButton"
           onclick="saveMember()">&nbsp;&nbsp; <input type="button"
                                                      value="关闭" class="ManagerButtonNo" onclick="closeWin()"/>
</div>
