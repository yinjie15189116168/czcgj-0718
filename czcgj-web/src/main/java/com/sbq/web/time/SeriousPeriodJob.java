...
/**
 * 严重告警定时器,默认5分钟一次,不入具体业务表,系统启动不加载,只加载正常定时器
 * 业务逻辑:
 * 判断是否告警:
 * -----如果不告警,则更新状态,关闭严重告警定时器,启动正常定时器
 * -----如果告警:
 * ---------获取当前状态是否被告警清除接口置为正常状态
 * ---------如果是正常状态：
 * -------------则在no_tip_minutes + last_warn_time内不再调用告警接口,如果超过时间范围,则更新状态为严重告警状态和告警时间,再调用一次严重告警接口
 * ---------如果状态还是严重告警状态:
 * -------------修改定时器时间为1分钟一次,重启严重告警定时器,调用一次严重告警接口</p>
 */
public class SeriousPeriodJob extends BaseService implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
        ...
        logger.info("SeriousPeriodJob-execute...." + DateUtil.now() + task_key);
                ...
                if (Math.abs(czcgLogDto.getX()) >= warn_angle || Math.abs(czcgLogDto.getY()) >= warn_angle || Math.abs(czcgLogDto.getZ()) >= warn_angle) {
                    ...
                    if (device.getLast_warn_status() == 0) {
                        ...
                        if (DateUtil.between(new Date(), time, DateUnit.SECOND) > 0) {
                            ...
                            //如果超过时间范围,则更新状态为严重告警状态和告警时间,再调用一次严重告警接口
...
//                        intervalQuartz.setStatus(1);
                        ...
                        intervalQuartz.setValue(1);//1分钟一次
